...
from models.modules import SelfAttention, CrossAttention, DeepResBlockDet, DeepResBlockDesc
...
class CVM(nn.Module):
    ...
    def __init__(self, device, grd_bev_res, grd_height_res, sat_bev_res, temperature=0.1, 
        ...
        # Ground Grid Queries
        self.grd_grid_queries = nn.Parameter(torch.rand(grd_bev_res, grd_bev_res, embed_dim), requires_grad=True)
        ...
        self.grd_attention_cross = nn.ModuleList([CrossAttention(device, grd_bev_res, grd_height_res, embed_dim, grid_size_h, grid_size_v) for _ in range(6)])
    ...
    def forward(self, grd_feature, sat_feature):
        ...
        grd_query = grd_query.permute(0, 2, 1).reshape(bs, self.embed_dim, self.grd_bev_res, self.grd_bev_res)

        # Ground descriptors & scores
        grd_desc = self.grd_projector(grd_query)
        grd_scrs = self.grd_detector(grd_query)
